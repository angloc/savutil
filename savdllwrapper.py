...
# Derived from http://code.activestate.com/recipes/577811-python-reader-writer-for-spss-sav-files/?in=user-4177640
#
# libspssdio.so.1, libspssdio.dylib, spssio32.dll + associated libaries and
# documentation can be downloaded here:
# https://www-01.ibm.com/marketing/iwm/iwm/web/reg/download.do?source=swg-tspssp&lang=en_US&S_PKG=do&cp=UTF-8&dlmethod=http
...
supportedDates = {  # really uses ISO dates wherever applicable.
    "DATE": "%Y-%m-%d",
    "JDATE": "%Y-%m-%d",
    "EDATE": "%Y-%m-%d",
    "SDATE": "%Y-%m-%d",
...
# Return an ISO 8601 representation for a time interval, with the highest designation
# being hours. 
def ISODuration (i, dp=0):
	if i is None: return
	if dp:
		seconds = i
		secondsInMinute = seconds % 60
		result = ("%0.*f" % (dp, secondsInMinute)).rstrip ('0').rstrip ('.')
	else:
		seconds = round (i)
		secondsInMinute = seconds % 60
		result = "%d" % secondsInMinute
	result += "S"
	minutes = math.floor (seconds / 60)
	if minutes:
		if result == "0S": result = ""
		wholeMinutes = math.floor (minutes % 60)
		if wholeMinutes or result: result = "%dM" % wholeMinutes + result
		hours = math.floor (minutes / 60)
		if hours:
			wholeHours = hours % 24
			if wholeHours or result: result = "%dH" % (hours % 24) + result
			days = math.floor (hours / 24)
			if days:
				result = "%dD" % days + result
	return "PT" + result
...
class Generic(object):
    ...
    def releaseInfo(self):
		...
		info = {
			...
			"release subnumber": 0,
			"fixpack number": 0,
			"big/little-endian code": 0	# Guess 
		}
	return info
...
class Header(Generic):
    ...
    def varLabels(self):
        ...
        varLabels = {}
        for varName in self.varNames:
            # Find the long label if exits
            lenBuff = MAXLENGTHS['SPSS_MAX_VARLABEL'] [0]
            varLabel = create_string_buffer (lenBuff + 1)
            labelLength = c_int()
            retcode = funcL(c_int(self.fh), c_char_p(varName),
                            byref(varLabel), c_int(lenBuff), byref(labelLength))
            varLabels[varName] = varLabel.value
            if retcode > 0:
                # Fallback to the short label
    ...
    def _splitformats(self):
        ...
        regex = re.compile("(?P<bareformat>[a-z]+)(?P<varWid>\d+)([.](?P<varDP>\d))?", re.I)
        bareformats, varWids, varDPs = {}, {}, {}
        for varName, format_ in self.formats.iteritems():
            match = regex.search(format_)
            bareformats[varName] = match.group ("bareformat").upper ()
            varWids[varName] = int(match.group("varWid"))
            if match.group ("varDP"):
            	varDPs[varName] = int(match.group("varDP"))
            else:
            	varDPs[varName] = 0
        return bareformats, varWids, varDPs
...
class SavReader(Header):
    ...
    def __init__(self, savFileName, returnHeader=False, recodeSysmisTo=None,
        ...
        self.bareformats, self.varWids, self.varDPs = self._splitformats()
    ...
    def formatValues(self, record):
        ...
        for i, value in enumerate(record):
            ...
            if varType == 0:
                ...
                elif bareformat_ == "DTIME":
                    record[i] = ISODuration (value, varDP)
            elif varType > 0:
                value = value[:varType]
                if self.ioUtf8_:
                    try:
                        ...
                        # Occasionally Unicode substitution characters appear at the end when
                        # multi-byte characters have been truncated. Remove these.
                    	record [i] = value.decode("utf-8", errors="replace")\
                    		.rstrip (u"\ufffd")
                    	#print "--Truncated utf-8 value for %s (%d): '%s'" %\
                    	#   (varName, i, record [i].encode ("ascii", errors="xmlcharrefreplace"))
                else:
                    record[i] = value
        return record
    ...
    def getHeader(self, selectVars=None):
...
class SavWriter(Header):
    ...
    * Optional (the associated SPSS commands are given in CAPS)
    ...
    def __init__(self, savFileName, varNames, varTypes, valueLabels=None,
        ...
        self.unicodeTruncationErrors = 0
